...
ISA: Olá, como posso te ajudar?\n`
  ...
  getPrompt = ({ clientText, phoneNumber }: { clientText: string, phoneNumber: string }) => {
    const parsedClientText = `${clientText}\nISA:`
    const prompt = `${this.contextMap.get(phoneNumber) || this.defaultContext + "\nHuman: "}${parsedClientText}`
    return prompt
  }
