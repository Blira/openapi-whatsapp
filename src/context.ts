...
export const getPrompt = ({ clientText, phoneNumber }: { clientText: string, phoneNumber: string }) => {
  const parsedClientText = `${clientText}\nISA:`
  const prompt = `${contextMap.get(phoneNumber) || defaultContext + "\nHuman: "}${parsedClientText}`
  return prompt
}

export const updateContext = ({ parsedApiTextResponse, phoneNumber, prompt }: { parsedApiTextResponse: string, phoneNumber: string, prompt: string }) => {
  const newContext = `${prompt}${parsedApiTextResponse}\nHuman: `
  contextMap.set(phoneNumber, newContext)
}

export const getContext = (phoneNumber: string) => {
  return contextMap.get(phoneNumber)
}

export const deleteContext = (phoneNumber: string) => {
  return contextMap.delete(phoneNumber)
}
